...
//        };
//        boardGrid = testBoard;
        initialize(boardGrid, boardSize);
        initialize(boardOverlay, boardSize);
    }

    public Board(int[][] board, int boardSize){
        BOARD_SIZE = boardSize;
	boardGrid = board;
	boardOverlay = new int[BOARD_SIZE][BOARD_SIZE];
    }
    ...
    public Board(int[][] board){
        boardGrid = board;
        boardOverlay = new int[board.length][board.length];
    }
    
    public Board(String board, int boardSize){
        for(int i = 0; i < boardSize; i++){
            for (int j = 0; j < boardSize; j++){
                boardGrid[i][j] = (int) board.charAt(i+j);
            }
        }
    }

    private void initialize(int[][] array, int boardSize){
        BOARD_SIZE = boardSize;
    ...
    public static Color getColor(int color){
        switch (color){
            ...
            case BLOCKED: return Color.GRAY;
